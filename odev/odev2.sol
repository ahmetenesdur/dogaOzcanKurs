...
contract digitalIdentity {
    ...
    function newIdentity(string memory name, string memory surname,uint tckn) public returns (uint identityId){
        identityId = identityCount++;
        identities[identityId] = Identity(tckn,identityPerson(name,surname));

    }

    function getIdentity(uint id) public view returns(string memory name,string memory surname, uint tckn){
